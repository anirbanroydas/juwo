...
class LivestreamApiHandler {
	
	constructor() {
		this.nsp = null;
		this.exchange = 'koatest_livestream_ex';
		this.exchange_type = 'topic';
		this.routing_key = 'livestream.';
	...
	ack_livestream(socket) {
		let self = this;
		
		// when the client emits 'ack_livestream'
		socket.on('ack_livestream', function () {
			let userinfo = socket.client.userinfo;
			
			log.debug('user acked livestream : ', userinfo.userid);
	...
	on_rabbitmq_message(msg) {
        ...
        try {
	        ...
	        if (this.socket) {
	        	...
	        	this.socketio.emit('livestream', body);
	        } 	        
		...
		let postAuthenticate_Generator = function *(socket, data, self) {
			...
			try {
				...
				let rabbitmq_config_options = {
			        ...
			        queue: 'koatest-queue-livestream-' + userid + '-' + utils.genid(8),
			        queue_opts: {
			        	durable: false,
			        	autoDelete: true,
			        	exclusive: true
...
module.exports = LivestreamApiHandler;
