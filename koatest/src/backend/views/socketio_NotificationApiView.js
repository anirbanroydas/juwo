...
// Handler for Socketio Notification Api Handler
class NotificationApiHandler {
	
	constructor() {
		this.nsp = null;
		this.exchange = 'koatest_notification_ex';
		this.exchange_type = 'topic';
		this.routing_key = 'notification.';
	...
	ack_notification(socket) {
		let self = this;
		
		// when the client emits 'ack_notification', we decreatese notification count from redis cache
		socket.on('ack_notification', function () {
			
			let ack_notification_Generator = function *() {

				let userinfo = socket.client.userinfo;
			
				// decreatse the notificaion count in redis cache
				let notification_count_key = 'koatest:notification:count:user:' + userinfo.userid;
				
				let redisDecr = yield redis.decrAsync(notification_count_key);
				log.debug('redisDecr : ', redisDecr);
			}

			let ack_notification_Coroutine = bluebird.coroutine(ack_notification_Generator);

			// run the coroutine
			ack_notification_Coroutine();			
	...
	on_rabbitmq_message(msg) {
        ...
        try {
         	// first try to decrease the notification count from redis cache 
	  		// let userinfo = socket.client.userinfo;
	        ...
	        // now send the notificaiton to socketio
	        if (this.socket) {
	        	// send the msg/notification via the corresponding socketio connection
	        	this.socketio.emit('notification', body);
	        } 	        
		...
		let postAuthenticate_Generator = function *(socket, data, self) {
			...
			try {
				...
				let rabbitmq_config_options = {
			        ...
			        queue: 'koatest-queue-notification-' + userid + '-' + utils.genid(8),
			        queue_opts: {
			        	durable: false,
			        	autoDelete: true,
			        	exclusive: true
...
module.exports = NotificationApiHandler;
