...
// Handler/Controller to handle route POST requests to Timeline User
const TimelineUserPostHandler = function *(next) {
    this.log.debug('Inside User Timeline Post Handler'); 
    ...
    let errorhandler = false;
  	...
  	if (!this.state.isAuthenticated){
	    ...
	    return;
      	...
      	let isValid = validator.isValidTimelineUserRequestData(this);
		...
		// get feed from redis
		let data = yield cache.get_timeline_user(sess.info.userid, this.request.body.cursor, this.request.body.count);
...
exports.TimelineUserPostHandler = TimelineUserPostHandler;
